...
class DataGrid
{
    ...
    public $tableWidth;
    public $defaultAlphabeticalSortBy;
    public $ajaxMode = false;
    public $showExportCheckboxes = true;
    public $showActionArea = true;
    public $showChooseColumnsBox = true;
    public $allowResizing = true;
    public $globalStyle = '';
    public $showExportColumn = true;
    ...
    public static function get($indentifier, $parameters, $misc = 0)
    {
        ...
        if (isset($_REQUEST['dynamicArgument' . md5($indentifier)])) {
            foreach ($parameters as $index => $data) {
                ...
                if ($index === 'exportIDs') {
                    $exportIDsDecoded = urldecode($parameters['exportIDs']);
                    if (is_string($exportIDsDecoded)) {
                        if (self::isSerialized($exportIDsDecoded)) {
                            $parameters['exportIDs'] = unserialize($exportIDsDecoded);
                        } else {
                            try {
                                $parameters['exportIDs'] = json_decode($exportIDsDecoded, true, 512, JSON_THROW_ON_ERROR);
                            } catch (JsonException $e) {
                                throw new JsonException('Invalid JSON provided for exportIDs. Data: ' . $exportIDsDecoded);
                            }
                        }
                    } else {
                        throw new JsonException('Invalid type provided for exportIDs, expected a string.');
                    }
                }
            }
        }
        ...
        if (isset($indentifierParts[2])) {
            if (is_string($indentifierParts[2])) {
                try {
                    $misc = json_decode($indentifierParts[2], true, 512, JSON_THROW_ON_ERROR);
                } catch (JsonException $e) {
                    throw new JsonException('Invalid JSON provided in identifier parts. Data: ' . $indentifierParts[2]);
                }
            } else {
                throw new JsonException('Invalid type provided in identifier parts, expected a string.');
            }
        }

        if (!file_exists(sprintf('modules/%s/dataGrids.php', $module))) {
            trigger_error('No datagrid named: ' . $indentifier, E_USER_ERROR);
        }

        include_once(sprintf('modules/%s/dataGrids.php', $module));

        if (!class_exists($class)) {
            throw new RuntimeException(sprintf('Class "%s" does not exist in module "%s".', $class, $module));
        }
    ...
    /**
     * Checks if a given value is a serialized string.
     *
     * @param string $data
     * @return bool
     */
        ...
        public function getMiscArgument()
        {
         ...
         public function __construct($instanceName, $parameters, $misc = 0)
         {
             ...
             if (isset($_GET['parameters' . $this->_instanceName])) {
                 $this->_parameters = json_decode($_GET['parameters' . $this->_instanceName], true, 512, JSON_THROW_ON_ERROR);
             } else {
